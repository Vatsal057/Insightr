...
/// Claim verifiability badges — FACT / OPINION / UNVERIFIED
class ClaimBadge extends StatelessWidget {
  final String verifiability;

  const ClaimBadge({super.key, required this.verifiability});

  @override
  Widget build(BuildContext context) {
    final (bg, fg, border) = _claimColors();
    return Container(
  ...
  (Color, Color, Color) _claimColors() {
    return switch (verifiability.toLowerCase()) {
      'fact' => (
          const Color(0x265CB870),
          const Color(0xFF5CB870),
          const Color(0x405CB870),
        ),
      'opinion' => (
          const Color(0x266A9AD4),
          InsightrColors.tagBlue,
          const Color(0x406A9AD4),
        ),
      _ => ( // unverified
          const Color(0x26D47850),
          const Color(0xFFD47850),
          const Color(0x40D47850),
        ),
    };
  }
}

/// Missing context badges — RISK / ASSUMPTION / TRADE-OFF etc.
class MissingBadge extends StatelessWidget {
  final String category;

  const MissingBadge({super.key, required this.category});

  @override
  Widget build(BuildContext context) {
    final (bg, fg, border) = _missingColors();
    return Container(
      padding: const EdgeInsets.symmetric(vertical: 2, horizontal: 7),
      decoration: BoxDecoration(
        color: bg,
  ...
  (Color, Color, Color) _missingColors() {
    return switch (category.toLowerCase()) {
      'risk' => (
          const Color(0x1FE05C4A),
          InsightrColors.red,
          const Color(0x33E05C4A),
        ),
      'assumption' || 'trade_off' || 'limitation' => (
          const Color(0x1F9A6AD4),
          InsightrColors.tagPurple,
          const Color(0x339A6AD4),
        ),
      _ => (
          const Color(0x1F6A9AD4),
          InsightrColors.tagBlue,
          const Color(0x336A9AD4),
        ),
    };
  }
}
